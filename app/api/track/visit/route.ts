...
/**
 * Public endpoint for logging anonymous variant visits.
 *
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    await logVisit(body.experimentId, body.variantType, body.visitorId);

    return NextResponse.json({ success: true });
  } catch (error) {
    return createErrorResponse(
      error instanceof Error ? error.message : "Unable to track visit.",
      "TRACKING_ERROR",
      500
    );
  }
